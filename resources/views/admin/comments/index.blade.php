...
<div class="CardBox">
    ...
    <div class="Card">
        <div>
            ...
            <div class="CardName">Semua Penilaian</div>
        </div>
        <div class="iconBox">
    ...
    <div class="Card">
        <div>
            ...
            <div class="CardName">Penilaian Aktif</div>
    ...
    <div class="Card">
        <div>
            ...
            <div class="CardName">Arsip Penialaian</div>
        </div>
        <div class="iconBox">
                            ...
                            <div class="table-responsive">
                                <table>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{$comment->id}}</td>
                                            <td class="text-center">{{Str::limit($comment->komen,70)}}</td>
                                            ...
                                            <td class="d-flex flex-row justify-content-center align-items-center ">
                                                ...
                                                <form id="{{$comment->id}}" action="{{route("reviews.update",$comment->id)}}" method="Post">
                                                    @csrf
                                                    @method("PUT")
                                                    <button onclick="event.preventDefault();
                                                            document.getElementById({{$comment->id}}).submit();" class="btn  btn-pr  btn-sm ml-2">
                                                ...
                                                <form id="{{$comment->id}}" action="{{route("reviews.destroy",$comment->id)}}" method="post" style="margin-left: 4px !important">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button class="btn btn-danger btn-sm" title="Archive this Review" onclick="event.preventDefault();
                                                ...
                                                @else
                                                <form id="{{$comment->id}}" action="{{route("review.unarchive",$comment->id)}}" method="Post">
                                                    @csrf
                                                    @method("PUT")
                                                    <button title="Unarchive this Review" onclick="event.preventDefault();
                                                            document.getElementById({{$comment->id}}).submit();" class="btn  btn-pr  btn-sm ml-2">
